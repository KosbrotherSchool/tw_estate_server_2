...
namespace :parse do 
	...
	task :clean_last_months_data => :environment do
		...
		Realestate.where("exchange_date >= 10210").each do |estate|
		...
		Realestate.where("exchange_date >= 10210").delete_all
	...
	task :parse_data => :environment do
		...
		1.upto 200 do |item_num| 
			...
			address = ""
	 		...
	 		# theRealEstate.estate_web_id = i, no use here
			...
			raw_item = RawItem.where(" raw_page_id = #{raw_page.id} And item_num = #{item_num - 1} ").first
			...
			begin
				buildingTable = page_no_detail.css("td.popup_box")[2].children[0]
			...
			park_data_size = 0
			begin
				parktable = page_for_table.css("table#park_data")
